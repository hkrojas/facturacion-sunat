...
// COMPONENTE ACTUALIZADO: Corregidas las rutas de importación y ajustados anchos de columna Estado/Acciones.
...
import EditModal from './EditModal'; // Corregido: Ruta dentro de components
import ConfirmModal from './ConfirmModal'; // Corregido: Ruta dentro de components
...
// FacturaDetailsModal (sin cambios funcionales)
                            ...
                            <div className="space-y-4 rounded-lg bg-gray-50 dark:bg-gray-700/50 p-4">
                                <DetailItem label="Estado">
                                    <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold ${sunatResponse.success ? 'bg-green-100 text-green-800 dark:bg-green-800/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-800/50 dark:text-red-300'}`}>
                                        {sunatResponse.success ? <CheckCircleIcon className="-ml-0.5 mr-1.5 h-4 w-4 text-green-500" /> : <XMarkIcon className="-ml-0.5 mr-1.5 h-4 w-4 text-red-500" />}
...
// ActionIcon (sin cambios funcionales)
const ActionIcon = ({ icon: IconComponent, color, onClick, disabled = false, tooltip }) => (
     <div className="relative group flex justify-center">
        <button onClick={onClick} disabled={disabled} className={`p-2 rounded-full transition-all duration-200 ease-in-out transform hover:scale-110 focus:outline-none focus:ring-2 focus:ring-offset-1 dark:focus:ring-offset-gray-800 ${color} ${disabled ? 'opacity-50 cursor-not-allowed hover:scale-100' : ''}`}>
...
const CotizacionesList = ({ refreshTrigger }) => {
    ...
    const fetchCotizaciones = async () => {
        if (!token) { setError("No autenticado. Por favor, inicie sesión de nuevo."); setLoading(false); return; }
        setLoading(true); setError('');
        try {
            const response = await fetch(`${API_URL}/cotizaciones/`, { headers: { 'Authorization': `Bearer ${token}` } });
            if (!response.ok) { let errorMsg = `Error ${response.status}: ${response.statusText}`; try { const errData = await response.json(); errorMsg = parseApiError(errData) || errorMsg; } catch (jsonError) { try { const textError = await response.text(); if(textError) errorMsg = textError; } catch(textErr) { /* fallback */ } } throw new Error(errorMsg); }
            const data = await response.json(); setCotizaciones(data);
        } catch (err) { console.error("Error fetching cotizaciones:", err); const errorToShow = err.message || "No se pudieron cargar las cotizaciones."; setError(errorToShow); addToast(errorToShow, 'error'); }
        finally { setLoading(false); }
    };

    useEffect(() => { fetchCotizaciones(); }, [token, refreshTrigger]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    const filteredCotizaciones = cotizaciones.filter(cot => (cot.nombre_cliente?.toLowerCase() || '').includes(searchTerm.toLowerCase()) || (cot.numero_cotizacion || '').includes(searchTerm));
                            ...
                            <tbody className="divide-y divide-gray-200 dark:divide-gray-700 text-gray-900 dark:text-gray-100">
                                {filteredCotizaciones.map((cot, index) => {
                                    ...
                                    <tr key={cot.id} className="transition-colors duration-200 hover:bg-gray-100 dark:hover:bg-gray-700/50 even:bg-gray-50 dark:even:bg-gray-800/50 staggered-fade-in-up" style={{ '--stagger-delay': `${index * 50}ms` }}>
                                        ...
                                        <td className="px-4 py-4 text-sm truncate-cell"> {/* Aplicada clase truncate-cell */}
                                        ...
                                        <td className="px-4 py-4 whitespace-nowrap text-center text-xs">
                                            {cot.comprobante ? ( <span className={`px-2 py-0.5 inline-flex leading-5 font-semibold rounded-full ${cot.comprobante.success ? 'bg-green-100 text-green-800 dark:bg-green-800/50 dark:text-green-300' : 'bg-red-100 text-red-800 dark:bg-red-800/50 dark:text-red-300'}`}> {cot.comprobante.success ? 'Fact.' : 'Rech.'} </span> ) : ( <span className="px-2 py-0.5 inline-flex leading-5 font-semibold rounded-full bg-gray-100 text-gray-800 dark:bg-gray-600 dark:text-gray-300">Pend.</span> )}
                                        </td>
                                        <td className="px-4 py-4 whitespace-nowrap text-center">
                                            <div className="flex justify-center items-center flex-wrap gap-x-2 gap-y-1"> {/* Ajustado gap */}
                                                ...
                                                {!cot.comprobante && cot.tipo_documento === 'RUC' && ( <ActionIcon onClick={() => handleFacturar(cot.id, 'factura')} disabled={isAnyLoading} tooltip="Emitir Factura" color="bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400 hover:bg-purple-200 dark:hover:bg-purple-700" icon={facturaIcon} /> )}
                                                {!cot.comprobante && ( <ActionIcon onClick={() => handleFacturar(cot.id, 'boleta')} disabled={isAnyLoading} tooltip="Emitir Boleta" color="bg-teal-100 dark:bg-teal-900/50 text-teal-600 dark:text-teal-400 hover:bg-teal-200 dark:hover:bg-teal-700" icon={boletaIcon} /> )}
