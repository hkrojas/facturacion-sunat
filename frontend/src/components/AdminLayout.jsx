...
// COMPONENTE ACTUALIZADO: Añadidas transiciones a los enlaces del sidebar.
...
const AdminLayout = ({ children }) => {
  ...
  // Añadido 'transition-colors duration-200' a linkStyle
  ...
  // Modificado estilo inactivo para definir colores base y hover
  const inactiveLinkStyle = "text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"; // Cambiado hover:bg-gray-200 a hover:bg-gray-100 para un efecto más sutil
    ...
    <div className="relative min-h-screen md:flex bg-gray-100 dark:bg-gray-900">
      ...
      <aside className={`fixed inset-y-0 left-0 bg-white dark:bg-gray-800 w-64 transform ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:relative md:translate-x-0 transition-transform duration-300 ease-in-out z-30 flex flex-col border-r dark:border-gray-700`}>
        <div className="flex items-center justify-between h-16 px-4 border-b dark:border-gray-700 flex-shrink-0">
          ...
          <button className="md:hidden text-gray-500 hover:text-gray-700 dark:hover:text-gray-300" onClick={() => setSidebarOpen(false)}> {/* Añadido dark:hover */}
            <CloseIcon />
          </button>
        </div>
        <nav className="flex-1 p-4 space-y-2">
          {/* NavLink ya aplica las clases correctamente */}
        ...
        <div className="p-4 border-t dark:border-gray-700">
          {/* Añadido transition-colors al enlace de volver */}
          <Link to="/dashboard" className="w-full text-center block px-4 py-2 text-sm text-blue-600 dark:text-blue-400 hover:underline hover:text-blue-800 dark:hover:text-blue-300 transition-colors duration-200">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white dark:bg-gray-800 border-b dark:border-gray-700 flex items-center justify-between md:justify-end px-6 flex-shrink-0">
          ...
          <button className="md:hidden text-gray-600 dark:text-gray-300 hover:text-gray-800 dark:hover:text-gray-100" onClick={() => setSidebarOpen(true)}> {/* Añadido hover */}
            <MenuIcon />
          </button>
          <ThemeToggle />
        </header>
        {/* Añadir animate-fade-in al contenido principal para entrada suave */}
